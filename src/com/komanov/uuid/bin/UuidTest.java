...
/**
 * A copy from http://hg.openjdk.java.net/jdk8/jdk8/jdk/file/default/test/java/util/UUID/UUIDTest.java
 * (actual revision: http://hg.openjdk.java.net/jdk8/jdk8/jdk/file/687fd7c7986d/test/java/util/UUID/UUIDTest.java)
 * <p>
 * Copyright notice:
 * <p>
...
public class UuidTest
{
    ...
    private final scala.Function1<String, UUID> fromStringF;

    public UuidTest(Function1<String, UUID> fromStringF)
    {
        this.fromStringF = fromStringF;
    }

    private UUID fromString(String s)
    {
        return fromStringF.apply(s);
    }
    ...
    private void randomUUIDTest() throws Exception
    {
        List list = new LinkedList();
        for (int i = 0; i < 100; i++)
        {
    ...
    private void nameUUIDFromBytesTest() throws Exception
    {
    ...
    private void stringTest() throws Exception
    {
        ...
        {
            UUID u1 = UUID.randomUUID();
            UUID u2 = fromString(u1.toString());
            if (!u1.equals(u2))
                throw new Exception("UUID -> string -> UUID failed");
        }
    }

    private void versionTest() throws Exception
    {
        ...
        test = fromString("9835451d-e2e0-1e41-8a5a-be785f17dcda");
        if (test.version() != 1)
            throw new Exception("wrong version fromString 1");
        test = fromString("9835451d-e2e0-2e41-8a5a-be785f17dcda");
        if (test.version() != 2)
            throw new Exception("wrong version fromString 2");
        test = fromString("9835451d-e2e0-3e41-8a5a-be785f17dcda");
        if (test.version() != 3)
            throw new Exception("wrong version fromString 3");
        test = fromString("9835451d-e2e0-4e41-8a5a-be785f17dcda");
        if (test.version() != 4)
    ...
    private void variantTest() throws Exception
    {
        UUID test = UUID.randomUUID();
        if (test.variant() != 2)
    ...
    private void timestampTest() throws Exception
    {
        UUID test = UUID.randomUUID();
        try
        {
        ...
        test = fromString("00000001-0000-1000-8a5a-be785f17dcda");
        if (test.timestamp() != 1)
            throw new Exception("Incorrect timestamp");
        test = fromString("00000400-0000-1000-8a5a-be785f17dcda");
        if (test.timestamp() != 1024)
            throw new Exception("Incorrect timestamp");
        test = fromString("FFFFFFFF-FFFF-1FFF-8a5a-be785f17dcda");
        if (test.timestamp() != Long.MAX_VALUE >> 3)
            throw new Exception("Incorrect timestamp");
    }

    private void clockSequenceTest() throws Exception
    {
        UUID test = UUID.randomUUID();
        try
        {
        ...
        test = fromString("00000001-0000-1000-8001-be785f17dcda");
        if (test.clockSequence() != 1)
            throw new Exception("Incorrect sequence");
        test = fromString("00000001-0000-1000-8002-be785f17dcda");
        if (test.clockSequence() != 2)
            throw new Exception("Incorrect sequence");
        test = fromString("00000001-0000-1000-8010-be785f17dcda");
        if (test.clockSequence() != 16)
            throw new Exception("Incorrect sequence");
        test = fromString("00000001-0000-1000-bFFF-be785f17dcda");
    ...
    private void nodeTest() throws Exception
    {
        ...
        test = fromString("00000001-0000-1000-8001-000000000001");
        if (test.node() != 1)
            throw new Exception("Incorrect node");
        test = fromString("00000001-0000-1000-8002-FFFFFFFFFFFF");
        if (test.node() != ((2L << 47) - 1)) // 2^48 - 1
            throw new Exception("Incorrect node");
    }

    private void hashCodeEqualsTest() throws Exception
    {
